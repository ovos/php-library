...
local function store_batches(n, batch_size)
...
redis.register_function('[prefix]store_batches', store_batches)

-- Splits a string by a given delimiter
local function store_split_string(str, delimiter)
	local result = {}
	local start = 1
...
redis.register_function('[prefix]store_split_string', store_split_string)

-- Scans redis for keys matching a specified prefix and processes them with a callback
local function store_scan_keys(prefix, count, callback)
	local cursor = '0'
	repeat
...
redis.register_function('[prefix]store_scan_keys', store_scan_keys)

-- Scans a redis hash for fields using HSCAN and processes them with a callback
local function store_hscan_keys(hash_key, count, callback)
	local cursor = '0'
	repeat
...
local function store_get_tags(keys, args)
	...
	store_scan_keys(prefix_tag_ids, 5000, function(full_tag_key)
...
{
	function_name = '[prefix]store_get_tags',
	callback = store_get_tags,
	flags = {'no-writes'}
}

-- Returns a list of all ids in a given tag
local function store_get_ids_by_tag(keys, args)
	local prefix = args[1]
	local tag = args[2]
	...
	store_hscan_keys(prefix_tag_ids .. tag, 5000, function(field)
...
redis.register_function
{
	function_name = '[prefix]store_get_ids_by_tag',
	callback = store_get_ids_by_tag,
	flags = {'no-writes'}
}

-- Unlink (remove) items by their ids and remove references from all tags
local function store_unlink_clean_tags(keys, args)
	local ids = keys
	local prefix = args[1]
	...
	for _, id in ipairs(ids) do
		...
		if item_tags then -- not false = item & hash field exist
			if item_tags ~= '' then -- not an empty string
				local tags = store_split_string(item_tags, ',')
...
redis.register_function('[prefix]store_unlink_clean_tags', store_unlink_clean_tags)

-- Unlink items by a given tag only if they match provided ids
local function store_unlink_ids_by_tag(keys, args)
	local ids = keys
	local prefix = args[1]
	local tag = args[2]
...
redis.register_function('[prefix]store_unlink_ids_by_tag', store_unlink_ids_by_tag)

-- Unlink all items from a given tag (removes every ID that tag references)
local function store_unlink_by_tag(keys, args)
	local prefix = args[1]
	local tag = args[2]
	local prefix_ids = prefix .. args[3]
	local prefix_tag_ids = prefix .. args[4]
...
local function store_unlink_all(keys, args)
	local prefix = args[1]
	local prefix_ids = prefix .. args[2]
	local prefix_tag_ids = prefix .. args[3]
	...
	-- unlink every ID
	local ids = {}
	store_scan_keys(prefix_ids, 5000, function(key)
		table.insert(ids, key)
	end)
	
	if #ids > 0 then
		for from, to in store_batches(#ids) do
	...
	-- unlink every tag
	local tags = {}
	store_scan_keys(prefix_tag_ids, 5000, function(tag_key)
		table.insert(tags, tag_key)
	end)
	
	if #tags > 0 then
		for from, to in store_batches(#tags) do
...
redis.register_function('[prefix]store_unlink_all', store_unlink_all)

-- Removes references from a tag that no longer has valid items
local function store_clean_tag(keys, args)
	local prefix = args[1]
	local tag = args[2]
	local prefix_ids = prefix .. args[3]
	local prefix_tag_ids = prefix .. args[4]
...
local function store_clear(keys, args)
	local prefix = args[1]
	...
	local ids = {}
	store_scan_keys(prefix, 5000, function(key)
		table.insert(ids, key)
	end)
	
	if #ids > 0 then
		count = count + #ids
		for from, to in store_batches(#ids) do -- security measure, not really needed with 5000 batch size
			redis.call('UNLINK', unpack(ids, from, to))
		end
	end
...
redis.register_function('[prefix]store_clear', store_clear)

-- Renew a lock if we still own it
local function store_renew_lock(keys, args)
...
redis.register_function('[prefix]store_renew_lock', store_renew_lock)

-- Release a lock and publish a notification
local function store_release_lock_and_publish(keys, args)
	local lock_key = keys[1]
...
redis.register_function('[prefix]store_release_lock_and_publish', store_release_lock_and_publish)
